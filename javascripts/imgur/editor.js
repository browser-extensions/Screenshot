...
function uploadWithImmio(website, callback) {
	ajaxObj = $.ajax({
		...
		timeout: 45000,
		data: 'name=screenshot&image='+encodeURIComponent(showCanvas.toDataURL()),
...
function uploadImage(website, callback) {
	...
	try {
			...
			if (website == "ultraimg") {
					ajaxObj = $.ajax({
					url: "http://smrtshot.com/api/1/upload",
					type: "POST",
					timeout: 125000,
					data:{key:"4355a79a868879822b0c324f0400388a", source: (SavePage.getImageSrc().replace(/^data:image\/(png|jpg);base64,/, ""))},
					//data:{key:"3374fa58c672fcaad8dab979f7687397", source: canvas.toDataURL()},
                    dataType: "json",
					success: function(data) {
                        if(data.status_code == 200) {
                            var imageURL = data.image.url_viewer;
							var DeleteURL = data.image.url_viewer;
			...
			ajaxObj = $.ajax({
				...
				type: "POST",
				timeout: 125000,
				data: "image=" + encodeURIComponent(SavePage.getImageSrc().replace(/^data:image\/(png|jpg);base64,/, "")),
				beforeSend : function(xhr) {
					xhr.setRequestHeader("Authorization", "Client-ID " + "8667c1317ba4b10");
				},
				complete: function(request, textStatus) {
					var status = getStatus(request, textStatus);
					if (status == 200) {
			...
			Minus.createGallery(function(gallery) {
				...
				var bin = atob(showCanvas.toDataURL().replace(/^data:image\/(png|jpg);base64,/, "")); //file.data
...
function postGrantedUpload(website) {
	...
	$("#uploadWebsitesWrapper").show("fast", function() {			
...
$(document).ready(function() {
	...
	loadStorage().then(function() {
		...
		function postImage(callback) {
			...
			$("#previewImage").attr("src", showCanvas.toDataURL());
			setTimeout(function() {
		...
		function showOptions(response) {
			if (response == null || response.error) {
				...
				$("#editedImage").attr("src", showCanvas.toDataURL());
			}
			if (response == null) {
